...
function get_items($db, $is_open = false, $search_order){
  $sql = '
    SELECT
      item_id, 
      name,
  ...
  if($is_open === true){
    ...
    } else if($search_order === "low") {
      $sql = $sql . ' ORDER BY items.price ASC';
    } else if($search_order === "high") {
...
function get_open_items($db, $search_order){
  return get_items($db, true, $search_order);
}
